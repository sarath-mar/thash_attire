<template>
  <div class="ta-admin-offer-edit">
    <AppLoading v-if="loading" />

    <template v-else-if="offer">
      <AdminPageHeader
        :title="`Edit: ${offer.name}`"
        subtitle="Modify combo offer details"
      />

      <v-form ref="formRef" v-model="valid" @submit.prevent="handleSubmit">
        <div class="ta-admin-offer-edit__layout">
          <div class="ta-admin-offer-edit__main">
          ...
          <div class="ta-admin-offer-edit__sidebar">
            ...
            <v-card elevation="0" border rounded="lg" class="pa-4 ta-sticky-sidebar">
              ...
              <v-btn variant="text" block class="mt-2" :to="Routes.ADMIN_OFFERS">
                Cancel
              </v-btn>
            </v-card>
          </div>
...
const { offer, fetchOffer, updateOffer, loading, calcRegularTotal, calcSavings, calcSavingsPercentage } = useOffers()
...
const handleSubmit = async () => {
  ...
  const res = await updateOffer(offerId, offerData, itemsData)
  if (res) {
...
watch(offer, (val) => {
  if (val) {
    ...
    form.offer_price = val.offer_price
    form.start_date = val.start_date ? val.start_date.split('T')[0] : ''
    form.end_date = val.end_date ? val.end_date.split('T')[0] : ''
    form.status = val.status
    
    if (val.offer_items) {
      items.value = val.offer_items.map(i => ({
